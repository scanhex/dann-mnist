...
def gen_fgbgcolor_data(loader, img_size, cpr, noise=10.):
    ...
    for i in tqdm.tqdm(range(tot_iters), total=tot_iters):
        ...
        color_choice = np.empty(targets.shape[0], dtype=int)
        for label in range(10):
            label_inds = targets == label
            label_df = targets[targets == label]
            if len(label_df) == 0:
                continue
            if (len(cpr) == 1):
                color_choice[label_inds] = np.random.multinomial(1, cpr, len(label_df)).argmax(axis=1)
            else:
                color_choice[label_inds] = np.random.multinomial(1, probs[label], len(label_df)).argmax(axis=1)
...
np.save(dir_name + '/modified_train_x.npy', color_data_x)
np.save(dir_name + '/modified_train_y.npy', color_data_y)
np.save(dir_name + '/modified_train_color.npy', color_data_color)

color_data_x, color_data_y, color_data_color = gen_fgbgcolor_data(testloader, img_size=(3, 28, 28), cpr=(1,), noise=10.)
np.save(dir_name + 'modified_test_x.npy', color_data_x)
np.save(dir_name + 'modified_test_y.npy', color_data_y)
